...
- Entities are grouped across the inverter device and five sub-devices (PV, Battery, Meter, Energy, Backup), and their categories follow the Home Assistant review guide: measurements stay uncategorised, settings are `CONFIG`, and capability, version and raw-register data is `DIAGNOSTIC`.
